...
public class HomeView extends AbstractEntityView {
    ...
    Node viewNode;
    Property coursesProp, accountProp, filterProp;
    Entity appEntity;
    ...
    public static final ActionNode.Category ENTER_COURSE = new ActionNode.Category();
    ...
    public HomeView(Entity appEntity, Node viewNode) {
        ...
        coursesProp = appEntity.findProperty(MainWindow.courses);
        ...
        Container categoryKebabButton = createCategoryButton("Kebab", "kebab-icon.png", evt -> {
            updateFilter(Course.CATEGORY_KEBAB);
            evt.consume();
            ActionNode action = viewNode.getInheritedAction(HomeView.ENTER_SEARCH);
            if (action != null) {
                action.fireEvent(appEntity, HomeView.this);
        ...
        Container categoryAllButton = createCategoryButton("All", "all-icon.png", evt -> {
            updateFilter(Course.CATEGORY_ALL);
            evt.consume();
            ActionNode action = viewNode.getInheritedAction(HomeView.ENTER_SEARCH);
            if (action != null) {
                action.fireEvent(appEntity, HomeView.this);
        ...
        categoryContainer.addAll(categoryKebabButton,categoryAllButton);
        add(categoryContainer);

        Container popularCnt = new Container(new BorderLayout());
        popularCnt.setUIID("PopularCnt");
        Label popularLabel = new Label("Featured courses", "CategoryHeader");
        ...
        popularCnt.add(BorderLayout.CENTER, createFeaturedCoursesContainer(appEntity.getEntityList(coursesProp)));
        ...
        recommendCnt.add(BorderLayout.CENTER, createRecommendedCoursesContainer(appEntity.getEntityList(coursesProp)));
        add(recommendCnt);

        Container allCoursesContainer = new Container(new BorderLayout());
        allCoursesContainer.setUIID("AllCoursesCnt");
        Label allCoursesLabel = new Label("All Courses", "CategoryHeader");
        allCoursesContainer.add(BorderLayout.NORTH, allCoursesLabel);
        allCoursesContainer.add(BorderLayout.CENTER, createAllCoursesContainer(appEntity.getEntityList(coursesProp)));
        add(allCoursesContainer);
    ...
    private Container createFeaturedCoursesContainer(EntityList<Entity> courses) {
        Container popularCnt = new Container(new BoxLayout(BoxLayout.X_AXIS));
        popularCnt.setScrollableX(true);
        for (Entity course : courses) {
            popularCnt.add(new CoursePreview(course, viewNode));
        }
        return popularCnt;
    }

    private Container createRecommendedCoursesContainer(EntityList<Entity> courses) {
        Container recommendedCnt = new Container(new BoxLayout(BoxLayout.X_AXIS));
        recommendedCnt.setScrollableX(true);
        for (Entity course : courses) {
            recommendedCnt.add(new CoursePreview(course, viewNode));
        }
        return recommendedCnt;
    }

    private Container createAllCoursesContainer(EntityList<Entity> courses) {
        final int coursesCount = courses.size();
        final int landscapeRows = coursesCount % 2 == 0 ? coursesCount / 2 : coursesCount / 2 + 1;
        Container allCoursesContainert;
        if (isTablet()){
            allCoursesContainert = new Container(new GridLayout(landscapeRows, 2));
        }else{
            allCoursesContainert = new Container(new GridLayout(coursesCount, 1, landscapeRows, 2));
        }

        for (Entity course : courses) {
            allCoursesContainert.add(new CoursePreview(course, viewNode));
        }

        return allCoursesContainert;
