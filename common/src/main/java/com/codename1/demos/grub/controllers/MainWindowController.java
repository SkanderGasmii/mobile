...
public class MainWindowController extends FormController {
    ...
    public static final ActionNode enterCourse = UI.action();
    ...
    public MainWindowController(Controller parent, Entity appEntity, Node appNode) {
        ...
        addActionListener(enterCourse, evt -> {
            evt.consume();
            Entity courseEntity = evt.getEntity();
            new CourseController(this, courseEntity, appEntity.getEntity(MainWindow.profile), homeNode).getView().show();
        });
        ...
        addActionListener(addCompletedOrder, evt -> {
            ...
            Entity course = completedOrder.getEntity(CompletedOrder.course);
            if (course instanceof CourseModel){
                ((CourseModel) course).clearOrder();
            }
        ...
        addActionListener(orderAgain, evt -> {
            evt.consume();
            Entity completedOrder = evt.getEntity();
            CourseModel course = (CourseModel) completedOrder.getEntity(CompletedOrder.course);
            if (completedOrder.get(CompletedOrder.order) instanceof EntityList){
                ...
                for (Entity dish : dishesList){
                    course.addToOrder(dish);
                }
            }
            new CourseController(this, course, appEntity.getEntity(MainWindow.profile), homeNode).getView().show();
    ...
    private Node createHomeNode(){
        return new ViewNode(
                UI.actions(HomeView.ENTER_COURSE, enterCourse),
                ...
                UI.actions(CourseView.ADD_TO_FAVORITE, addFavorite),
                UI.actions(CourseView.REMOVE_FAVORITE, removeFavorite),
