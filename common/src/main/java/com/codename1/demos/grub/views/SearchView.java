...
public class SearchView extends AbstractEntityView {
    ...
    Node viewNode;
    Property coursesProp, filterProp;
    Entity appEntity;
    ...
    public SearchView(Entity appEntity, Node viewNode) {
        ...
        Label headerLabel = new Label("COURSES", "SearchHeaderLabel");
        ...
        add(filterCnt);
        Label categoryHeaderLabel = new Label("All Courses", "CategoryHeader");
        add(categoryHeaderLabel);
        final int coursesCount = ((EntityList)appEntity.get(coursesProp)).size();
        final int landscapeRows = coursesCount % 2 == 0 ? coursesCount / 2 : coursesCount / 2 + 1;
        if(CN.isTablet()){
            coursesContainer = new Container(new GridLayout(landscapeRows, 2));
        }else{
            coursesContainer = new Container(new GridLayout(coursesCount, 1, landscapeRows, 2));
        }
    ...
    @Override
    public void update() {
        if(appEntity.get(coursesProp) instanceof EntityList){
            EntityList<Entity> courses = (EntityList)appEntity.get(coursesProp);
            for (Entity course : courses) {
                coursesContainer.add(new CoursePreview(course, viewNode));
            }
        }
    }
    ...
    private Container create(EntityList<Entity> courses) {
