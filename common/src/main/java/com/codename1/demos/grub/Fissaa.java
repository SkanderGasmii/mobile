...
public class Fissaa extends ApplicationController {
    Entity account;
        ...
        if (evt instanceof StartEvent) {
            ...
            if (mode == DEBUG_MODE_WITH_SIGNING || mode == DEBUG_MODE_WITH_SIGNING_DARK_MODE){
                ...
                addActionListener(enterMainWindow, event->{
                    event.consume();
                    new MainWindowController(this, createMainWindowEntity(account), viewNode).getView().show();
                });
                ...
                new MainWindowController(this, createMainWindowEntity(null), viewNode).getView().show();
    ...
    private Entity createMainWindowEntity(Entity accountEntity){
        Entity account;
        if (accountEntity == null){
             account = new AccountModel();
            account.set(Account.firstName, "skander");
            account.set(Account.lastName, "gasmi");
        ...
        for (int i = 0; i < 10; i++){
            coursesList.add(createCourseDemoModel());
        }
        return new MainWindowModel(account, coursesList);
    }

    private Entity createCourseDemoModel(){
        CourseModel course = new CourseModel("Sergio's Pizza", "https://sergeycodenameone.github.io/restaurant.png", "Italian", 4.7, 5, "https://sergeycodenameone.github.io/restaurant-icon.png",30, createDemoMenu());
        return course;
    }
