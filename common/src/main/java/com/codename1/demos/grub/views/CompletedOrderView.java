...
public class CompletedOrderView extends AbstractEntityView {
    ...
    public CompletedOrderView(Entity order, Node viewNode) {
        ...
        String courseName = order.getEntity(CompletedOrder.course).getText(Course.name);

        Label courseNameLabel = new Label(courseName, "CompletedOrderRestName");
        ...
        CourseModel course = (CourseModel) order.getEntity(CompletedOrder.course);

        PropertySelector imagePropertySelector = new PropertySelector(course, Course.icon);
        RoundRectImageRenderer renderer = new RoundRectImageRenderer(100, 100, 1);

        Image courseImage = renderer.createImage(imagePropertySelector);
        ScaleImageLabel courseIconLabel = new ScaleImageLabel(courseImage);
        ...
        orderTopView.add(BorderLayout.NORTH, BorderLayout.centerEastWest(BoxLayout.encloseY(courseNameLabel, orderNumber), completedIcon, courseIconLabel));
