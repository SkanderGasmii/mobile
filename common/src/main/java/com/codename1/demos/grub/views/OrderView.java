...
public class OrderView extends AbstractEntityView {
    ...
    public OrderView(Entity courseEntity, Entity profileEntity, Node viewNode, Node mainWindowNode) {
        super(courseEntity);
        ...
        orderProp = courseEntity.findProperty(Course.order);
        ...
        if (courseEntity.get(orderProp) instanceof EntityList) {
            EntityList<Entity> dishList = (EntityList) (courseEntity.get(Course.order));
        ...
        Label deliveryFeeLabel = new Label(courseEntity.getDouble(Course.deliveryFee) + " " + L10NManager.getInstance().getCurrencySymbol(), "OrderDeliveryFee");
        ...
        confirmOrder.addActionListener(evt->{
            ...
            if (action != null) {
                ...
                action.fireEvent(new CompletedOrderModel(courseEntity, (EntityList)courseEntity.get(Course.order), date, Address.HOME), OrderView.this);
            }
        });
    ...
    @Override
    public void update() {
        itemTotalCostLabel.setText(Util.getPriceAsString(((CourseModel)getEntity()).getTotalItemPrice()));
        totalPriceLabel.setText(Util.getPriceAsString(((CourseModel)getEntity()).getTotalPrice()));
