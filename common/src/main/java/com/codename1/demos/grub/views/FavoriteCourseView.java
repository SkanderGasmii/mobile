...
public class FavoriteCourseView extends AbstractEntityView {

    private static EncodedImage placeHolder = EncodedImage.createFromImage(getGlobalResources().getImage("placeholder.png"), false);

    public FavoriteCourseView(CourseModel course, Node viewNode) {
        super(course);
        setUIID("FavoriteRestView");
        setLayout(new BorderLayout(BorderLayout.CENTER_BEHAVIOR_CENTER));
        Label dishesNumber = new Label(course.getDishesCount() + " Dishes", "DishesCount");
        ...
        PropertySelector imagePropertySelector = new PropertySelector(course, Course.picture);
        RoundRectImageRenderer courseImageRenderer = new RoundRectImageRenderer(1000, 1000, 2);

        Image courseImage = courseImageRenderer.createImage(imagePropertySelector);
        ...
        courseImageButton.addActionListener(evt -> {
            ...
            if (action != null) {
                action.fireEvent(course, FavoriteCourseView.this);
            }
        });
        setLeadComponent(courseImageButton);
        courseImageButton.setBackgroundType(Style.BACKGROUND_IMAGE_SCALED_FILL);

        Container courseImageContainer = new Container(new BorderLayout()){
            @Override
        ...
        courseImageContainer.setUIID("FavoriteRestImageCnt");
        courseImageContainer.add(BorderLayout.SOUTH, dishesNumber).add(BorderLayout.CENTER, courseImageContainer);

        EntityList<Entity> dishes;
        Image firstDish, secondDish, thirdDish;
        if(course.getEntity(Course.menu) instanceof EntityList){
            EntityList<Entity> categories = (EntityList<Entity>) course.getEntity(Course.menu);
            if (categories.get(0).get(FoodCategory.dishes) instanceof EntityList){
                ...
                add(BorderLayout.CENTER, courseImageContainer).
                        add(BorderLayout.NORTH, new Label(course.getText(Course.name), "FavoriteRestName")).
